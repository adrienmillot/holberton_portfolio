...
const buildDomProposal = function (label, i, id) {
  const proposal_row = '#proposal_row_' + (i - 1);
  if (i === 1 || i % 2 != 0) {
...
const proposalColLeft = function (label, i, id) {
  const defInput = $('<input type="checkbox" />').attr('data-id', id);
  const defProp = $('<p class="checkbox"></p>').append(defInput).append(' ' + label);
  const defCol = $('<div class="col-sm-5"></div>').append(defProp);
  // var closeDiv = $('</div>')
...
const proposalColRight = function (label, id) {
  const defInput = $('<input type="checkbox" />').attr('data-id', id);
  const defProp = $('<p class="checkbox" id="checkbox_1"></p>').append(defInput).append(' ' + label);
