...
function buildPaginationBtnsSurvey (page_count, page) {
  ...
  for (i = 1; i <= page_count; i++) {
    ...
    const linkAction = $('a#' + i + '_survey.page-link');
...
function surveyActionsButton (survey) {
  const showSurveyButton = surveyShowButton(survey);
  const editSurveyButton = surveyEditButton(survey);
  const deleteSurveyButton = surveyDeleteButton(survey);
...
$(document).ready(function () {
  const page = $('#page_argument_survey').val();
