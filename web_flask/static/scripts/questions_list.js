...
function questionRow(question, count) {
	...
	var CategoryidTd = $('<td></td>').text(GetCategoryNameForQuestion(question.category_id));
	var SurveyidTd = $('<td></td>').text(GetSurveyNameForQuestion(question.survey_id));
...
function deleteActionQuestion(id) {
	...
	$.ajax({
		url: 'https://ss-api.2835holberton.tech/api/v1/questions/' + id,
		type: 'DELETE',
		headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` },
		error: function (data) {
			dataResponse = data.responseJSON
